...
//Typing effect
else if (currentChar < string_length(dialoguePage))
{
...
//show_debug_message((nameString!=""&&keyboard_check(ord("X"))))
	...
	var _punc = string_char_at(dialoguePage, currentChar);
	...
	for (var pu = 0; pu < 4; pu++)
	{
		...
		{
			currentChar++
			_punc = string_char_at(dialoguePage, currentChar);
		...
		var _keystring = string_char_at(dialoguePage, currentChar);
	
		if (_keystring == "/")
		{
			currentChar++
			_keystring = string_char_at(dialoguePage, currentChar);
			...
			//	show_debug_message($"char: {currentChar}");
				...
				var _dig1 = string_char_at(dialoguePage, currentChar);
				var _dig2 = string_char_at(dialoguePage, currentChar+1);
			...
			//	show_debug_message($"char: {currentChar}");
			currentChar++;
			while(string_char_at(dialoguePage, currentChar) != "~") {
				_name = string_concat(_name, string_char_at(dialoguePage, currentChar))
				currentChar++;
				_timer++;
				if(_timer > 100) {
					exit
...
//-------Set points where the dialogue wraps-------//
    var _words = string_split(dialoguePage, " ");
    var _wrappedText = "";
    var _currentLine = "";
